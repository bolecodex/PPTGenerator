...
api_ep="https://9y4phvjly3.execute-api.us-east-1.amazonaws.com/default/myChatGPTfunction"
city = st.text_input('City', 'Beijing')


# headers for request and response encoding, same for both endpoints
headers = {"accept": "application/json", "Content-Type": "application/json"}
data = {"location": city}
resp = req.post(api_ep, headers=headers, json=data)
if resp.status_code != 200:
    output = resp.text
else:
    output = resp.json()['body']
